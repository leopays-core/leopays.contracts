...
BOOST_FIXTURE_TEST_CASE( buysell, eosio_system_tester ) try {
   ...
   transfer( "lpc", "alice1111111", core_sym::from_string("1000.0000"), "lpc" );
   BOOST_REQUIRE_EQUAL( success(), stake( "lpc", "alice1111111", core_sym::from_string("200.0000"), core_sym::from_string("100.0000") ) );

   auto total = get_total_stake( "alice1111111" );
   auto init_bytes =  total["ram_bytes"].as_uint64();

   const asset initial_ram_balance = get_balance(N(lpc.ram));
   const asset initial_ramfee_balance = get_balance(N(lpc.ramfee));
   ...
   BOOST_REQUIRE_EQUAL( initial_ram_balance + core_sym::from_string("199.0000"), get_balance(N(lpc.ram)) );
   BOOST_REQUIRE_EQUAL( initial_ramfee_balance + core_sym::from_string("1.0000"), get_balance(N(lpc.ramfee)) );
   ...
   transfer( "lpc", "alice1111111", core_sym::from_string("100000000.0000"), "lpc" );
...
BOOST_FIXTURE_TEST_CASE( stake_unstake, eosio_system_tester ) try {
   ...
   const auto init_eosio_stake_balance = get_balance( N(lpc.stake) );
   ...
   BOOST_REQUIRE_EQUAL( init_eosio_stake_balance + core_sym::from_string("300.0000"), get_balance( N(lpc.stake) ) );
   ...
   produce_blocks(1);
   BOOST_REQUIRE_EQUAL( core_sym::from_string("1000.0000"), get_balance( "alice1111111" ) );
   BOOST_REQUIRE_EQUAL( init_eosio_stake_balance, get_balance( N(lpc.stake) ) );
...
BOOST_FIXTURE_TEST_CASE( stake_unstake_with_transfer, eosio_system_tester ) try {
   ...
   //lpc stakes for alice with transfer flag

   transfer( "lpc", "bob111111111", core_sym::from_string("1000.0000"), "lpc" );
   ...
   transfer( "lpc", "alice1111111", core_sym::from_string("1000.0000"), "lpc" );
   BOOST_REQUIRE_EQUAL( success(), stake( "alice1111111", "alice1111111", core_sym::from_string("200.0000"), core_sym::from_string("100.0000") ) );
   //now alice's stake should be equal to transfered from lpc + own stake
   total = get_total_stake("alice1111111");
   BOOST_REQUIRE_EQUAL( core_sym::from_string("700.0000"), get_balance( "alice1111111" ) );
...
BOOST_FIXTURE_TEST_CASE( fail_without_auth, eosio_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), stake( "lpc", "alice1111111", core_sym::from_string("2000.0000"), core_sym::from_string("1000.0000") ) );
...
BOOST_FIXTURE_TEST_CASE( producer_register_unregister, eosio_system_tester ) try {
   ...
   auto key =  fc::crypto::public_key( std::string("LPC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV") );
   ...
   auto key2 =  fc::crypto::public_key( std::string("LPC5jnmSKrzdBHE9n8hw58y7yxFWBC8SNiG7m8S1crJH3KvAnf9o6") );
   BOOST_REQUIRE_EQUAL( success(), push_action(N(alice1111111), N(regproducer), mvo()
                                               ("producer",  "alice1111111")
                                               ("producer_key", key2 )
                                               ("url", "http://block.two")
...
BOOST_FIXTURE_TEST_CASE( producer_wtmsig_transition, eosio_system_tester ) try {
   ...
   // authority which would be rejected by the LeoPays native system and cause the onblock transaction to continue to fail.
...
BOOST_FIXTURE_TEST_CASE( vote_for_producer, eosio_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), unstake( "carol1111111", core_sym::from_string("2.0000"), core_sym::from_string("0.0002")/*"2.0000 LPC", "0.0002 LPC"*/ ) );
...
BOOST_FIXTURE_TEST_CASE(producer_pay, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   // produce enough blocks so new schedule kicks in and defproducera produces some blocks
   {
      ...
      const int64_t  initial_savings           = get_balance(N(lpc.saving)).get_amount();
      ...
      const int64_t  savings           = get_balance(N(lpc.saving)).get_amount();
...
BOOST_FIXTURE_TEST_CASE(change_inflation, eosio_system_tester) try {
...

   {
      ...
      auto run_for_1year = [this](int64_t annual_rate, int64_t inflation_pay_factor, int64_t votepay_factor) {
         ...
         const int64_t final_savings = get_balance(N(lpc.saving)).get_amount();
...
BOOST_AUTO_TEST_CASE(extreme_inflation) try {
   ...
   t.create_currency( N(lpc.token), config::system_account_name, asset((1ll << 62) - 1, core_symbol) );
   ...
   {
      vector<char> data = t.get_row_by_account( N(lpc.token), name(core_symbol.to_symbol_code().value), N(stat), account_name(core_symbol.to_symbol_code().value) );
...
BOOST_FIXTURE_TEST_CASE(multiple_producer_pay, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
...

   {
      ...
      const uint32_t initial_tot_unpaid_blocks = initial_global_state["total_unpaid_blocks"].as<uint32_t>();
      const asset    initial_supply            = get_token_supply();
      const asset    initial_bpay_balance      = get_balance(N(lpc.bpay));
      const asset    initial_vpay_balance      = get_balance(N(lpc.vpay));
      ...
      const uint32_t tot_unpaid_blocks = global_state["total_unpaid_blocks"].as<uint32_t>();
      const asset    supply            = get_token_supply();
      const asset    bpay_balance      = get_balance(N(lpc.bpay));
      const asset    vpay_balance      = get_balance(N(lpc.vpay));
...

   {
      ...
      BOOST_REQUIRE_EQUAL( error("missing authority of lpc"),
...
BOOST_AUTO_TEST_CASE(votepay_transition2, * boost::unit_test::tolerance(1e-10)) try {
   ...
   std::string old_contract_core_symbol_name = "LPC"; // Set to core symbol used in contracts::util::system_wasm_old()
...
BOOST_FIXTURE_TEST_CASE( elect_producers /*_and_parameters*/, eosio_system_tester ) try {
   ...
   transfer( "lpc", "alice1111111", core_sym::from_string("600000000.0000"), "lpc" );
...
BOOST_FIXTURE_TEST_CASE( buyname, eosio_system_tester ) try {
   ...
   transfer( "lpc", "nofail", core_sym::from_string( "1000.0000" ) );
...
BOOST_FIXTURE_TEST_CASE( multiple_namebids, eosio_system_tester ) try {
   ...
   {
      const asset initial_names_balance = get_balance(N(lpc.names));
      BOOST_REQUIRE_EQUAL( success(),
      ...
      BOOST_REQUIRE_EQUAL( initial_names_balance + core_sym::from_string("0.1001"), get_balance(N(lpc.names)) );
   }
...
BOOST_FIXTURE_TEST_CASE( eosioram_ramusage, eosio_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( false, get_row_by_account( N(lpc.token), N(alice1111111), N(accounts), account_name(symbol{CORE_SYM}.to_symbol_code()) ).empty() );

   //remove row
   base_tester::push_action( N(lpc.token), N(close), N(alice1111111), mvo()
                             ( "owner", "alice1111111" )
                             ( "symbol", symbol{CORE_SYM} )
   );
   BOOST_REQUIRE_EQUAL( true, get_row_by_account( N(lpc.token), N(alice1111111), N(accounts), account_name(symbol{CORE_SYM}.to_symbol_code()) ).empty() );

   auto rlm = control->get_resource_limits_manager();
   auto eosioram_ram_usage = rlm.get_account_ram_usage(N(lpc.ram));
   auto alice_ram_usage = rlm.get_account_ram_usage(N(alice1111111));

   BOOST_REQUIRE_EQUAL( success(), sellram( "alice1111111", 2048 ) );

   //make sure that ram was billed to alice, not to lpc.ram
   BOOST_REQUIRE_EQUAL( true, alice_ram_usage < rlm.get_account_ram_usage(N(alice1111111)) );
   BOOST_REQUIRE_EQUAL( eosioram_ram_usage, rlm.get_account_ram_usage(N(lpc.ram)) );
...
BOOST_FIXTURE_TEST_CASE( ram_gift, eosio_system_tester ) try {
   ...
   */
   ...
   BOOST_REQUIRE_EQUAL( success(), unstake( "lpc", "alice1111111", core_sym::from_string("20.0000"), core_sym::from_string("10.0000") ) );
...
BOOST_FIXTURE_TEST_CASE( rex_auth, eosio_system_tester ) try {
   ...
   const asset one_lpc      = core_sym::from_string("1.0000");
   ...
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(deposit), mvo()("owner", alice)("amount", one_lpc) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(withdraw), mvo()("owner", alice)("amount", one_lpc) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(buyrex), mvo()("from", alice)("amount", one_lpc) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg),
                        push_action( bob, N(unstaketorex), mvo()("owner", alice)("receiver", alice)("from_net", one_lpc)("from_cpu", one_lpc) ) );
   ...
   BOOST_REQUIRE_EQUAL( error(error_msg),
                        push_action( bob, N(rentcpu), mvo()("from", alice)("receiver", alice)("loan_payment", one_lpc)("loan_fund", one_lpc) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg),
                        push_action( bob, N(rentnet), mvo()("from", alice)("receiver", alice)("loan_payment", one_lpc)("loan_fund", one_lpc) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(fundcpuloan), mvo()("from", alice)("loan_num", 1)("payment", one_lpc) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(fundnetloan), mvo()("from", alice)("loan_num", 1)("payment", one_lpc) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(defcpuloan), mvo()("from", alice)("loan_num", 1)("amount", one_lpc) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(defnetloan), mvo()("from", alice)("loan_num", 1)("amount", one_lpc) ) );
   ...
   BOOST_REQUIRE_EQUAL( error("missing authority of lpc"), push_action( alice, N(setrex), mvo()("balance", one_lpc) ) );
...
BOOST_FIXTURE_TEST_CASE( unstake_buy_rex, eosio_system_tester, * boost::unit_test::tolerance(1e-10) ) try {
...

   {
      ...
      const asset init_eosio_stake_balance = get_balance( N(lpc.stake) );
      ...
      BOOST_REQUIRE_EQUAL( tot_stake,                               get_balance( N(lpc.rex) ) );
      BOOST_REQUIRE_EQUAL( tot_stake,                               init_eosio_stake_balance - get_balance( N(lpc.stake) ) );
...
BOOST_FIXTURE_TEST_CASE( ramfee_namebid_to_rex, eosio_system_tester ) try {
   ...
   asset cur_ramfee_balance = get_balance( N(lpc.ramfee) );
   BOOST_REQUIRE_EQUAL( success(),                      buyram( alice, alice, core_sym::from_string("20.0000") ) );
   BOOST_REQUIRE_EQUAL( get_balance( N(lpc.ramfee) ), core_sym::from_string("0.1000") + cur_ramfee_balance );
   ...
   cur_ramfee_balance = get_balance( N(lpc.ramfee) );
   asset cur_rex_balance = get_balance( N(lpc.rex) );
   ...
   BOOST_REQUIRE_EQUAL( cur_ramfee_balance,                get_balance( N(lpc.ramfee) ) );
   BOOST_REQUIRE_EQUAL( get_balance( N(lpc.rex) ),       cur_rex_balance + core_sym::from_string("0.3500") );

   cur_rex_balance = get_balance( N(lpc.rex) );
   ...
   BOOST_REQUIRE_EQUAL( core_sym::from_string("23.7000"), get_balance( N(lpc.names) ) );
   BOOST_REQUIRE_EQUAL( success(),                        bidname( alice, N(rndmbid), core_sym::from_string("29.3500") ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("29.3500"), get_balance( N(lpc.names) ));
   ...
   BOOST_REQUIRE_EQUAL( get_balance( N(lpc.rex) ),      cur_rex_balance + core_sym::from_string("34.3500") );
   BOOST_REQUIRE_EQUAL( 0,                                get_balance( N(lpc.names) ).get_amount() );
...
BOOST_FIXTURE_TEST_CASE( rex_lower_bound, eosio_system_tester ) try {
   ...
   double rex_per_lpc = double(tot_rex) / double(tot_lendable);
   int64_t sell_amount = rex_per_lpc * ( tot_unlent - 0.09 * tot_lent );
   produce_block( fc::days(5) );
   BOOST_REQUIRE_EQUAL( success(), sellrex( alice, asset( sell_amount, rex_sym ) ) );
   BOOST_REQUIRE_EQUAL( success(), cancelrexorder( alice ) );
   sell_amount = rex_per_lpc * ( tot_unlent - 0.1 * tot_lent );
   BOOST_REQUIRE_EQUAL( success(), sellrex( alice, asset( sell_amount, rex_sym ) ) );
...
BOOST_AUTO_TEST_CASE( setabi_bios ) try {
   ...
   t.create_account(N(lpc.token));
   t.set_abi( N(lpc.token), contracts::token_abi().data() );
   {
      auto res = t.get_row_by_account( config::system_account_name, config::system_account_name, N(abihash), N(lpc.token) );
      _abi_hash abi_hash;
   ...
   t.set_abi( N(lpc.token), contracts::system_abi().data() );
   {
      auto res = t.get_row_by_account( config::system_account_name, config::system_account_name, N(abihash), N(lpc.token) );
      _abi_hash abi_hash;
      auto abi_hash_var = abi_ser.binary_to_variant( "abi_hash", res, base_tester::abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( setabi, eosio_system_tester ) try {
   set_abi( N(lpc.token), contracts::token_abi().data() );
   {
      auto res = get_row_by_account( config::system_account_name, config::system_account_name, N(abihash), N(lpc.token) );
      _abi_hash abi_hash;
   ...
   set_abi( N(lpc.token), contracts::system_abi().data() );
   {
      auto res = get_row_by_account( config::system_account_name, config::system_account_name, N(abihash), N(lpc.token) );
      _abi_hash abi_hash;
      auto abi_hash_var = abi_ser.binary_to_variant( "abi_hash", res, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( change_limited_account_back_to_unlimited, eosio_system_tester ) try {
   BOOST_REQUIRE( get_total_stake( "lpc" ).is_null() );

   transfer( N(lpc), N(alice1111111), core_sym::from_string("1.0000") );

   auto error_msg = stake( N(alice1111111), N(lpc), core_sym::from_string("0.0000"), core_sym::from_string("1.0000") );
   auto semicolon_pos = error_msg.find(';');

   BOOST_REQUIRE_EQUAL( error("account lpc has insufficient ram"),
   ...
   push_action( N(lpc), N(setalimits), mvo()
                                          ("account", "lpc")
   ...
   stake( N(alice1111111), N(lpc), core_sym::from_string("0.0000"), core_sym::from_string("1.0000") );

   REQUIRE_MATCHING_OBJECT( get_total_stake( "lpc" ), mvo()
      ("owner", "lpc")
   ...
   BOOST_REQUIRE_EQUAL( success(),
                        push_action( N(lpc), N(setacctnet), mvo()
                           ("account", "lpc")
                           ("net_weight", -1)
                        )
   );
   ...
   BOOST_REQUIRE_EQUAL( success(),
                        push_action( N(lpc), N(setacctcpu), mvo()
                           ("account", "lpc")
                           ("cpu_weight", -1)
...
BOOST_FIXTURE_TEST_CASE( buy_pin_sell_ram, eosio_system_tester ) try {
   ...
   transfer( N(lpc), N(alice1111111), core_sym::from_string("1020.0000") );

   auto error_msg = stake( N(alice1111111), N(lpc), core_sym::from_string("10.0000"), core_sym::from_string("10.0000") );
   auto semicolon_pos = error_msg.find(';');
   ...
   BOOST_REQUIRE_EQUAL( success(), buyrambytes( N(alice1111111), N(lpc), static_cast<uint32_t>(ram_bytes_needed) ) );

   auto tokens_paid_for_ram = alice_original_balance - get_balance( N(alice1111111) );

   auto total_res = get_total_stake( "lpc" );
   ...
   BOOST_REQUIRE_EQUAL( success(),
                        push_action( N(lpc), N(setacctram), mvo()
                           ("account", "lpc")
                           ("ram_bytes", total_res["ram_bytes"].as_int64() )
                        )
   );

   auto lpc_original_balance = get_balance( N(lpc) );

   BOOST_REQUIRE_EQUAL( success(), sellram( N(lpc), total_res["ram_bytes"].as_int64() ) );

   auto tokens_received_by_selling_ram = get_balance( N(lpc) ) - lpc_original_balance;
