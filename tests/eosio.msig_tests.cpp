...
public:
   eosio_msig_tester() {
      create_accounts( { N(lpc.msig), N(lpc.stake), N(lpc.ram), N(lpc.ramfee), N(alice), N(bob), N(carol) } );
      produce_block();
      ...
      set_code( N(lpc.msig), contracts::msig_wasm() );
      set_abi( N(lpc.msig), contracts::msig_abi().data() );
   ...
   transaction_trace_ptr create_account_with_resources( account_name a, account_name creator, asset ramfunds, bool multisig,
      ...
      trx.actions.emplace_back( get_action( N(lpc), N(buyram), vector<permission_level>{{creator,config::active_name}},
                                            mvo()
                                            ("payer", creator)
                                            ("receiver", a)
                                            ("quant", ramfunds) )
      ...
      trx.actions.emplace_back( get_action( N(lpc), N(delegatebw), vector<permission_level>{{creator,config::active_name}},
                                            mvo()
                                            ("from", creator)
                                            ("receiver", a)
   ...
   asset get_balance( const account_name& act ) {
      ...
      const auto* tbl = db.find<table_id_object, by_code_scope_table>(boost::make_tuple(N(lpc.token), act, N(accounts)));
      share_type result = 0;
   ...
   transaction_trace_ptr push_action( const account_name& signer, const action_name& name, const variant_object& data, bool auth = true ) {
      ...
      auto trace = base_tester::push_action( N(lpc.msig), name, accounts, data );
      produce_block();
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_all_approve, eosio_msig_tester ) try {
   ...
   // required to set up the link between (lpc active) and (lpc.prods active)
   //
   //                  lpc active
   //                       |
   //             lpc.prods active (2/3 threshold)
   ...
   set_authority(
      ...
      authority( 1,
                 ...
                 vector<permission_level_weight>{{{N(lpc.prods), config::active_name}, 1}}
      ),
      config::owner_name,
   ...
   create_accounts( { N(lpc.token), N(lpc.rex) } );
   set_code( N(lpc.token), contracts::token_wasm() );
   set_abi( N(lpc.token), contracts::token_abi().data() );

   create_currency( N(lpc.token), config::system_account_name, core_sym::from_string("10000000000.0000") );
   ...
   BOOST_REQUIRE_EQUAL( core_sym::from_string("1000000000.0000"),
                        get_balance(config::system_account_name) + get_balance(N(lpc.ramfee)) + get_balance(N(lpc.stake)) + get_balance(N(lpc.ram)) );
   ...
   produce_blocks();
   create_account_with_resources( N(alice1111111), N(lpc), core_sym::from_string("1.0000"), false );
   create_account_with_resources( N(bob111111111), N(lpc), core_sym::from_string("0.4500"), false );
   create_account_with_resources( N(carol1111111), N(lpc), core_sym::from_string("1.0000"), false );
   ...
   vector<permission_level> action_perm = {{N(lpc), config::active_name}};
   ...
   // execute by alice to replace the lpc system contract
   transaction_trace_ptr trace;
   control->applied_transaction.connect(
   [&]( std::tuple<const transaction_trace_ptr&, const signed_transaction&> p ) {
      const auto& t = std::get<0>(p);
   ...
   BOOST_REQUIRE_EXCEPTION( create_account_with_resources( N(alice1111112), N(lpc), core_sym::from_string("1.0000"), false ),
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_major_approve, eosio_msig_tester ) try {
   ...
   // set up the link between (lpc active) and (lpc.prods active)
   set_authority(
      config::system_account_name,
      config::active_name,
      authority( 1,
...
BOOST_FIXTURE_TEST_CASE( approve_execute_old, eosio_msig_tester ) try {
   set_code( N(lpc.msig), contracts::util::msig_wasm_old() );
   set_abi( N(lpc.msig), contracts::util::msig_abi_old().data() );
   produce_blocks();

   //propose with old version of lpc.msig
   auto trx = reqauth( N(alice), {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
   push_action( N(alice), N(propose), mvo()
                  ("proposer",      "alice")
                  ("proposal_name", "first")
