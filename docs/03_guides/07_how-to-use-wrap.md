...
The transaction to create the `eosio.wrap` account will need to be proposed to get the necessary approvals from active block producers before executing it. This transaction needs to first be generated and stored as JSON into a file so that it can be used in the leopays-cli command to propose the transaction to the eosio.msig contract.
...
Three unsigned transactions will be generated using leopays-cli and then the actions within those transactions will be appropriately stitched together into a single transaction which will later be proposed using the eosio.msig contract.

First, generate a transaction to capture the necessary actions involved in creating a new account:
```
$ leopays-cli system newaccount -s -j -d --transfer --stake-net "1.000 LPC" --stake-cpu "1.000 LPC" --buy-ram-kbytes 50 eosio eosio.wrap EOS8MMUW11TAdTDxqdSwSqJodefSoZbFhcprndomgLi9MeR2o8MT4 > generated_account_creation_trx.json
...
```
$ leopays-cli push action -s -j -d eosio newaccount newaccount_payload.json -p eosio > generated_newaccount_trx.json
...
```
$ leopays-cli push action -s -j -d eosio setpriv '{"account": "eosio.wrap", "is_priv": 1}' -p eosio > generated_setpriv_trx.json
...
```
$ leopays-cli get account eosio.wrap
privileged: true
permissions:
...
The transaction to deploy the contract to the `eosio.wrap` account will need to be proposed to get the necessary approvals from active block producers before executing it. This transaction needs to first be generated and stored as JSON into a file so that it can be used in the leopays-cli command to propose the transaction to the eosio.msig contract.

The easy way to generate this transaction is using leopays-cli:
```
$ leopays-cli set contract -s -j -d eosio.wrap contracts/eosio.wrap/ > deploy_wrap_contract_trx.json
...
```
$ leopays-cli get code -a retrieved-eosio.wrap.abi eosio.wrap
...
This example will demonstrate how to use the deployed eosio.wrap contract together with the eosio.msig contract to allow a greater than two-thirds supermajority of block producers of an LeoPays blockchain to change the owner authority of an arbitrary account. The example will use leopays-cli: in particular, the `leopays-cli multisig` command, the `leopays-cli set account permission` sub-command, and the `leopays-cli wrap exec` sub-command. However, the guide also demonstrates what to do if the `leopays-cli wrap exec` sub-command is not available.
...
```
$ leopays-cli set account permission -s -j -d alice owner '{"threshold": 1, "accounts": [{"permission": {"actor": "eosio", "permission": "active"}, "weight": 1}]}' > update_alice_owner_trx.json
```
...
```
$ leopays-cli push action -s -j -d eosio.wrap exec '{"executer": "blkproducera", "trx": ""}' > wrap_update_alice_owner_trx.json
$ cat wrap_update_alice_owner_trx.json
{
...
The approvers should go through the human-readable transaction output and make sure everything looks fine. However, due to a current limitation of leopays-node/leopays-cli, the JSONification of action payload data does not occur recursively. So while both the `hex_data` and the human-readable JSON `data` of the payload of the `eosio.wrap::exec` action is available in the output of the `leopays-cli multisig review` command, only the hex data is available of the payload of the inner `eosio::updateauth` action. So it is not clear what the `updateauth` will actually do.
